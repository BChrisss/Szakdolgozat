...
for i = 1:Robotnum                                                          % Frissíti az érzékelt területek értékeit
    for m = 0:5:45
       for n = 0:5:45
            if norm(Robot{i}{2} - [m n 0]) <= distSensor || norm(Robot{i}{2} - [m+5 n 0]) < distSensor || norm(Robot{i}{2} - [m n+5 0]) < distSensor || norm(Robot{i}{2} - [m+5 n+5 0]) < distSensor
                ...
                for p = 1:Targetnum                                         % Célpont érzékelés esete
                ...
                if not(foundTarget)                                         % Nem érzékelt célpontot
...
for j = 1:10                                                                % Globális memóriamátrix frissítése
    for k = 1:10
        for l = 1:Robotnum
